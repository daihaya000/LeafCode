...
事前解析に使うモデルは OpenCode に登録済みのものへ一本化しています。画像事前解析は既定で有効な機能で、設定の「プロバイダー/モデル」タブ（「タイトル / NextAction 生成モデル」の下）で、接続済みプロバイダーが持つ画像入力対応モデルを選択すると自動で利用できます。認証情報は OpenCode 側の登録をそのまま使い、WebUI の設定ファイルへ API キーを複製しません。解析時はツールを無効化した一時セッションを作成し、応答を取得後に削除します。選択モデルが画像に対応している場合は事前解析を行わず、画像を直接送信します。

ローカルの Ollama も同じ経路で利用します。「プロバイダー/モデル」タブの「ローカルOllamaを登録」で `opencode.jsonc` へプロバイダー登録すると、画像対応モデルが事前解析モデルの候補に現れます。Ollama のインストールとモデル取得は別途行ってください（自動セットアップは廃止済み）。登録したモデルは OpenCode の再起動後に利用できます。
