...
:: Allow LeafCode via Caddy HTTPS (port 8443) through Windows Firewall.
...
netsh advfirewall firewall delete rule name="LeafCode Caddy HTTPS" >nul 2>&1
netsh advfirewall firewall add rule name="LeafCode Caddy HTTPS" dir=in action=allow protocol=TCP localport=8443 profile=any enable=yes
if errorlevel 1 (
