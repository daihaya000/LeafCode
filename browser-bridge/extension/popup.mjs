...
function render(state) { $('status').textContent = state.connected ? 'Connected to local Broker' : state.paired ? 'Paired; reconnecting…' : 'Not paired'; $('share').disabled = !state.paired; $('tabs').replaceChildren(...state.sharedTabs.map((tab) => { const item = document.createElement('li'); const button = document.createElement('button'); button.textContent = `Stop sharing ${tab.title || tab.origin}`; button.onclick = () => call('unshare', { tabId: tab.id }).then(render).catch(showError); item.append(button); return item; })); }
