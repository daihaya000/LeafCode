...
## 期待 / 実際
- 期待: Settings で通貨/為替表示を変えると他タブのコスト表示も追従する
- 実際: 同一タブは CustomEvent で更新されるが、他タブはリロードまで旧表示のまま

## 根本原因
- `useCostDisplayPrefs` が `COST_DISPLAY_EVENT` のみ購読。CustomEvent はタブを跨がない
- 権限/Auto 設定は `storage` も購読済みだがコスト prefs だけ欠けていた

## 修正
- `webui:cost-display`（または clear）の `storage` で `readCostDisplayPrefs` を再適用

## 回帰防止
- currency: updates when another tab writes cost-display via storage event
- currency: resets to defaults when another tab clears localStorage

## 検証
- vitest currency.test.ts 20 PASS / eslint OK

## 残存リスク
- Host BUILD_ID 初回待ち UX 体感、voice 入力は未着手（高影響の確定バグは未確認）
...
## タイトル/NextAction生成モデル
- `generation-model` 設定を追加し、プロバイダー/モデル設定画面から `provider::model` を指定可能にした。
- タイトル再生成と NextAction API は設定済みモデルを優先し、未設定時は従来のモデル選択へフォールバックする。
- 型チェックと対象 API テスト 98 件が通過。

## ループ
- 既存 3m `AGENT_LOOP_TICK_bughunt` を停止し、1m `AGENT_LOOP_TICK_bughunt1m`（PID 32212）を起動
- プロンプト: 徹底バグハント（ユーザー影響度優先）

## 期待 / 実際
- 期待: 「確認する」中にサブエージェント（子・孫）が起動しても、生成直後から edit/write/apply_patch は承認待ち
- 実際: 親の accessMode 再同期まで OpenCode 既定 `{ "*": "allow" }` のまま書けた。孫は直下 children 同期でも天井が届かなかった。別タブは CustomEvent のみで mode が食い違った

## 根本原因
- `setSessionEditPermission` は直下 children のスナップショット依存で、`session.created` に追従しない
- `/children` は再帰せず孫セッションを見落とす
- access/subagent/skill は同タブ `CustomEvent` のみで、`storage` 未購読

## 修正
- `session.created` でルート配下の descendant を追跡し `descendantAccessSync` を bump → TaskView が即 `/api/access-mode` 再 POST
- `listDescendantSessionIds`（深さ上限 8）で子・孫へ同じ edit 天井を適用
- `/api/access-mode` は bound parent の descendant id を受理
- TaskView / HomeView / GlobalAttention が `storage` でタブ間の mode を同期

## 回帰防止
- `shouldSyncAccessCeilingForSessionCreated` 単体
- `setSessionEditPermission` の孫への再帰 PATCH
- access-mode route の grandchild 受理
- TaskView: `descendantAccessSync` 増加で再同期

## 検証
- vitest: opencode-access-mode 11 / access-mode route 9 / TaskView access 系 / HomeView shared access 追従 PASS
- eslint 対象ファイル成功
- 本番ビルドは未実行（AGENTS.md）

## 残存リスク
- `session.created`〜PATCH 完了までの極短レースは残りうる（エンジン側 inheritance があればさらに縮む）
- 稼働中の本番 `.next` には反映されない（ユーザー明示のリビルド後に有効）

---

# 作業ログ: 承認 / フルアクセスの徹底デバッグ

## 日付
2026-08-12

## 期待 / 実際
- 期待: 「確認する」では edit/write/apply_patch が承認カード待ち。「フルアクセス」では自動承認。deny(task/skill) は full より優先。
- 実際: UI とエンジンの `edit` ルールが乖離し、確認する表示のまま無承認書き込み、またはフルなのにカードが出る／二重応答する経路があった。

## 根本原因
- TaskView / HomeView の `accessMode` 初期値が `"ask"` 固定で、localStorage=`full` の hydrate より先に `/api/access-mode` が `ask` を送れた（`TaskView.tsx` 旧 L689 + L1221）。
- `changeAccessMode` だけ楽観的に localStorage を書き、失敗時にロールバックしない（subagent/skill と不一致）。
- Home が `webui:access-mode` を購読せず、モーダルでフルにしたあと新規タスクが古い mode で作られた。
- 子セッションは bindings 外のため `edit` 天井が乗らず、OpenCode 既定 `{ "*": "allow" }` のまま書けた。
- 分割ペインの非アクティブ側が TaskView 自動応答と GlobalAttention の両方で reply した。
- SessionSwitcher 新規作成は prompt 可能前に権限同期せず、最初の apply_patch が無カードになり得た。
- Workflow 実行中の accessMode PATCH が node の write deny を last-match で上書きし得た。

## 修正
- 初期値を `readAccessMode()` / `readSubagentPermission()` / `readSkillPermission()` にし、Home は access/skill イベントも購読。
- `changeAccessMode` はエンジン ACK 後だけ `writeAccessMode`。失敗時は UI を戻す。
- `setSessionEditPermission` が直下の child session にも同じ `edit` ルールを適用。`/api/access-mode` は bound parent の child id を受け付ける。
- AttentionQueueModal のフル切替は既知タスクへ `/api/access-mode` を直接 PATCH。
- 非アクティブペインはカードも自動 reply も出さず GlobalAttention に任せる。
- SessionSwitcher.create 後、onSwitch 前に access/subagent/skill を同期。
- Workflow 実行中は accessMode 同期をスキップ。

## 回帰テスト
- TaskView: full hydrate が最初の PATCH / full 自動 approve / inactive split は reply しない / workflow 中は同期しない / PATCH 失敗で 確認する維持
- HomeView: `webui:access-mode` でセレクトが full に追従
- GlobalAttention: バックグラウンド権限を full で `once`
- AttentionQueueModal: フル切替時に `/api/access-mode` を呼ぶ
- SessionSwitcher: 新規セッション作成後に権限同期してから onSwitch
- `setSessionEditPermission` + `/api/access-mode`: child 適用 / unbound 404

## 検証
- 関連 Vitest 17 focused + TaskView 133 / AttentionQueueModal 15 / GlobalAttention 11 / SessionSwitcher 9 / access-mode route 8 / opencode-access-mode 7 PASS
- 対象 eslint 成功
- `tsc --noEmit` は既存 `web/src/lib/db.ts` の SQL コメント構文エラーで失敗（今回未変更）
- 本番ビルドはプロジェクト指示により未実行

## 残存リスク
- （更新）子生成直後ギャップとタブ間 storage は本 tick で対処。極短レースと本番未反映は残る。
- HomeView の submit/image 系テストは既存失敗が残っており、今回はイベント購読のみ回帰追加。

---

# 作業ログ: 徹底バグハント tick（/loop 3m）画像送信タイムアウト不一致

## 日付
2026-08-12

## ループ
- 既存 3 分ループ（PID 42624 / `AGENT_LOOP_TICK_bughunt`）を継続。重複起動なし
- プロンプト: 徹底バグハント（ユーザー影響度優先）

## 根本原因
- 画像解析の設定上限は 600s なのに、外側の待ち時間が短く、解析中に送信が失敗する
  - クライアント `IMAGE_ANALYSIS_SEND_TIMEOUT_MS` = 180s
  - 新規タスク `ocServer(prompt_async)` は timeout 未指定 → 暗黙 10s
  - 新規タスク hang-watch / コメント上の予算 = 60s
  - BFF `prompt_async` 上流 = 90s、route `maxDuration` = 300s
- 新規タスクの slash command も `ocServer` 暗黙 10s で、BFF の 290s より先に切れる
- Home 新規タスク（画像なし）は `DEFAULT_FETCH_TIMEOUT_MS` = 30s で、worktree/devcontainer の provision や command 完了前に UI が失敗する

## 修正
- 共有定数 `web/src/lib/image-send-timeout.ts`
  - VL max 600s + setup 30s = クライアント画像送信 630s
  - `prompt_async` 202 待ち 90s、新規タスク（画像なし）295s
  - route `maxDuration` を VL 予算以上へ
- `/api/tasks` の `prompt_async` / `command` に明示 timeout を渡す
- 画像解析設定の timeout を min/max でクランプ（API は範囲外を 400）

## 回帰防止
- `image-send-timeout.test.ts`: VL max を外側予算が覆うこと
- `tasks/route.test.ts`: prompt_async 90s / command 290s を `ocServer` に渡すこと
- `useSessionStream.test.ts`: 添付あり mutation が 600s 以上
- settings read/PUT: 過大 timeout をクランプ / 拒否

## 検証
- vitest 206 PASS（image-send-timeout / useSessionStream / settings / qwen-native settings / tasks / sse-health / client）
- eslint 対象ファイル / `tsc --noEmit` 成功

## 残
- 本番は host 再起動／リビルド後に有効
- HomeView 既存の submit/image 系テスト失敗は今回無関係

---

# 作業ログ: HomeView の effort 欄が表示されない不具合

## 日付
2026-08-12

## 対象
- Home コンポーザー（Claude Opus 5 + build。フルアクセス / 許可 / 禁止 / ループは表示されるが effort なし）

## 期待 / 実際
- 期待: モデル横に Intelligence（effort）セレクタが出る
- 実際: `intelligenceVariants.length === 0` のため非表示

## 証拠
- ライブ `GET /api/opencode/provider`: `anthropic/claude-opus-5` に `low|medium|high|xhigh|max`
- ライブ `GET /api/extensions/provider-models`: 同モデルの `variants` が空オブジェクト
- OpenAI など他プロバイダーも同様に空 → Anthropic 固有ではない

## 根本原因
- `normalizeProviderResponse`（`web/src/lib/opencode-extensions/provider-models.ts`）が OpenCode の `{ low: {} }` を `{ low: undefined }` に落としていた
- `JSON.stringify` が undefined 値を省略し、API 応答の variants キーが消える
- HomeView は `/api/extensions/provider-models` だけを見るため `getIntelligenceVariants` が `[]` を返しセレクタが消える
- ディスクキャッシュ v1 も同じ空 variants を保持していた

## 修正
- 宣言済み variant は `{}` のまま残す（`disabled` があるときだけ `{ disabled }`）
- `getIntelligenceVariants` はキー存在＋`disabled !== true` で有効とみなす
- ディスクキャッシュを v2 にして、空 variants 化した v1 を使わない

## 回帰テスト
- `provider-models.test.ts`: JSON round-trip 後も Claude Opus 5 の effort キーが残る / v1 キャッシュを無視
- `model-variants.test.ts`: `{ low: undefined }` でも有効

## 検証
- `vitest` provider-models 52 + model-variants 16 PASS
- 対象 eslint / `tsc --noEmit` 成功
- HomeView 全体は既存の submit/image 系 11 件失敗（今回無関係。effort 関連テストは成功）

## 反映
- 本番は host の production ビルド経由。再起動／リビルド後に有効
- ソース: `web/src/lib/opencode-extensions/provider-models.ts` / `.test.ts` / `model-variants.ts` / `.test.ts`

---

# 作業ログ: バグハントループ再開（/loop 3m）
...
### P1 別セッション送信が Goal loop を abort しない
- TaskView `send()` の PATCH pause を `goalLoop.sessionId === 送信先` のときだけ実行
- キュー drain もループ所有セッション以外ではブロックしない（sessionId 未設定時は従来どおりブロック）

### P1 abort 後にキューを自動送信しない
- 停止時に当該 composer scope を hold。明示送信まで auto-drain しない
- `abortAndHoldQueue` で hang 判定停止・停止ボタンを共通化

## 検証
- TaskView 128 + useSessionStream stuck-busy 6 = **134 PASS**

## 残
- 画像送信タイムアウト不一致（client 180s / VL 最大 600s / BFF prompt_async 90s / 新規タスク 60s）

---

# 終了済みセッションの hang-watchdog 誤再開を修正
# 作業ログ: Cursor resumeのエージェント分離
...
- OpenCode実ログで、同一session内の `agent=title`（small=true）と `agent=build` が約330ms差で並行開始していた。
- resumeキーは `workspace + model + sessionID + conversation anchor` だけだったため、両リクエストが同じキーへ別々のCursor chat IDを記録し、遅く完了した方がbuildの継続先を上書きできた。
- OpenCode plugin hookの正式型には `chat.headers.input.agent` があり、本文やsystem promptを推測分類する必要はない。

## 対策

- `input.agent` をSHA-256の32桁fingerprintにして `x-opencode-agent-fingerprint` でloopback proxyへ渡す。
- resumeキーを `workspace + model + sessionID + agentFingerprint + anchor` に変更。title / compaction / build / custom agentを独立laneとして扱う。
- Claude proxyは会話resumeを持たずprocess単位のsession headerのみ、CommandCode proxyは毎回全履歴送信であり、Cursor固有の共有resumeキャッシュだけに分離を実装した。

## 検証

- 「build完了後にtitleが遅れて記録」の回帰テストは修正前に失敗し、修正後成功。
- Cursor proxy全テスト25件、`node --check`、Web `tsc --noEmit` 成功。
- 専用portの一時OpenCode 1.18.14 serverで実測。1ターン目はtitle（tools=0）とbuild（tools=57）が並行し別chatを作成。2ターン目はbuild chat IDのhashへincremental resumeし、title chatのmtimeは不変、新規chatも増えなかった。
- full prompt 64,615文字に対し送信prompt 413文字、input 28,952→187、cache_read 5,888→34,816、cost $0.0378→$0.00908（約76%減）。
- 検証用OpenCode session・Cursor chat・一時server/logは削除済み。profile mainには新bundle（directory hash `406dcd0f...`）を配置済み。稼働hostへの反映は次回再起動後。
...
- OpenCode 経由の1ターン目で Cursor チャットが**388ms差で2個**作られた（`9c596b7f` と `7f6447f6`）。後のDB再確認で、小さい `9c596b7f` がtitle、大きく2ターン目にも更新された `7f6447f6` がbuildと判明した。
- したがって、この実測では2ターン目はbuild chatへresumeしていた。cache_readが41344→512、コストが0.0114→0.0523 USDへ増えた事実だけでは誤resumeの証拠にならない。ただしtitle/buildが同一resumeキーへ並行記録する競合自体はコードと回帰テストで再現した。
...
- セッション再開キーが workspace・model・最初のユーザー文だけで、OpenCode `sessionID` を含まない。同じ開始文の新規タスクが1時間以内の旧Cursor chatを再利用し、会話混線することを最小再現で確認した。
- 画像付き継続ターン等で差分プロンプトを作れない場合、`--resume` を維持したままフル履歴を送るため、保存済み履歴が二重化することを最小再現で確認した。
- resume固有エラー時はキャッシュを削除するだけで、そのリクエストをresumeなしで再試行せずエラー応答する。
- 反復ガードはtool引数の完全一致だけを数えるため、意味が同じで文字列が異なる確認コマンドを止めない。一方、正当な同一コマンドも3回目で無言終了する。

## 検証

- 同一開始文の新規リクエストが旧 `chatId` を取得する最小再現を実行
- 画像付き継続ターンが `resumeChatId` とフル履歴を同時に返す最小再現を実行
- Cursor proxy関連テスト4件は成功したが、上記境界ケースをカバーしていない
...
- 1ターン内で複数発生する完了済みassistantイベントを、セッション単位で500msの静穏期間に集約した。
- 抽出は最後のassistantメッセージIDだけで1回実行し、途中ステップごとの重複した抽出呼び出しを抑制する。
- 同一イベントの重複通知は従来どおり無視し、goal完了など明示的な直接スケジュール経路は変更していない。
- 複数ステップが1回の抽出になる回帰テストを追加した。

## 検証

- `npm test`: 275ファイル / 3301成功 / 1スキップ
- `npm run typecheck`: 成功
- `npm run lint -- src/lib/memory-auto-extract.ts src/lib/memory-auto-extract.test.ts`: 成功
...
- 自動抽出完了ごとに発生していた `memory-audit` の標準出力を既定で停止した。
- `memory_audit_log` への永続監査は維持し、調査時のみ `MEMORY_AUDIT_STDOUT=1` で標準出力を有効化できる。
- 既定で標準出力へ出さないこととDB監査が残ることを回帰テストで固定した。

## 検証

- `npm test`: 成功（1スキップ）
- `npm run typecheck`: 成功
- `npm run lint -- src/lib/memory.ts src/lib/memory.test.ts`: 成功
...
- 左半分へドロップした場合は左ペインだけ、右半分へドロップした場合は右ペインだけを差し替える。反対側のタスクを移動・置換しない。すでに左右どちらかへ表示中の同一タスクは重複ドロップ対象にしない。
- HomeViewも左ペインとして分割でき、Homeを維持したまま右へタスクを表示できる。Home上で左へドロップした場合は左だけタスクへ遷移し、既存の右ペインを維持する。
- 分割中に別タスクをドロップすると右ペインを置換する。右タスクのヘッダーから1ペインへ戻せる。
- 分割開始時は左右のTaskViewを自動的に「会話」タブへ切り替え、狭いペインでDiff/Workflowタブを開いたままにならないようにする。
...
- `AppShell` に左右のドロップ領域と等幅2ペインを追加した。左ドロップは左のルートだけを変更し、右ドロップは右タスク状態だけを変更する。HomeとTaskだけを分割ホストとして扱い、設定画面へ移動した場合は分割を終了する。右側の `TaskView` は動的importし、ホーム等の初期バンドルを増やさない。
...
- Web全体Vitest: 275ファイル / 3303成功 / 1スキップ。
- `npm run typecheck`: 成功。
- 変更対象ESLint: 成功。
...
# 作業ログ: EXE起動が next-build の pause で止まる不具合
...
- `web/src/app/api/opencode/[...path]/route.ts` と `web/src/app/api/tasks/route.ts` が `export const maxDuration = IMAGE_SEND_ROUTE_MAX_DURATION_SEC` と書いていた。
- Next.js 16.3 は segment config を静的リテラルのみ受け付け、インポート定数だと Collecting page data で `Invalid segment configuration export` になり本番ビルドが失敗する。
- host が stale rebuild で `.next` / `BUILD_ID` を消したあとビルド失敗すると、次回 EXE 起動は `start-webui.bat` の初回ビルド経路に入り、失敗後 `pause` で止まる。コンソールタイトルは next が上書きした `next-build (v16.3.0)` のまま残る。

## 修正

- 両ルートの `maxDuration` を数値リテラル `640` にした（`IMAGE_SEND_ROUTE_MAX_DURATION_SEC` と同じ値。Next.js がインポート値を拒否するため）。
- `image-send-timeout.ts` の定数も同じ 640 に固定し、ルートソースがリテラルと一致することをテストで固定した。
- `start-webui.bat` の `pause_if_interactive` でタイトルを `OpenCode WebUI` に戻す。

## 回帰防止

- `image-send-timeout.test.ts`: ルートが `export const maxDuration = 640` を含み、インポート定数を使わないこと。
- `start-webui-bat.test.js`: pause 前に `title OpenCode WebUI` があること。

## 検証

- `npm.cmd test -- --run src/lib/image-send-timeout.test.ts`: 6 tests成功
- `node --test src/start-webui-bat.test.js`（host）: 20 tests成功
- `npx eslint`（変更TS）: 成功
- `npm.cmd run typecheck`（web）: 成功
- `npm.cmd run test:encoding`: 7 tests成功
- 本番 `next build` は AGENTS.md により未実行。ユーザーは pause 中の EXE を閉じて `OpenCodeWebUI.exe` を再起動する。

## 残存リスク

- `BUILD_ID` 欠落時は初回ビルドが可視コンソールで走る。成功すればホスト起動に進むが数分かかる。
- Turbopack の dynamic filesystem access 警告は未解消（今回の失敗原因ではない）。
