...
## 2026-07-24 バグハントループ終了 — actionable バグ枯渇

### 結果
`/loop 2m` で約12サイクル。権限同期・画像上限・Attention/フルアクセス・intelligence など高優先バグを修正。最終探索（HEAD `b2ad129`）は CLEAN。ループ停止。

**主なコミット:** `64a2a1f` … `b2ad129`
...
### やったこと
直近コミット `ed8405b` のバグハントで発見した問題を修正。

**問題:**
- モバイル（`!isMd`）でタスクが working 状態の場合、ヘッダーの停止ボタンが非表示になるが、ケバブメニューにも停止ボタンがなかった
- 結果として、モバイルユーザーは実行中のタスクを停止できなかった

**修正:**
- `TaskView.tsx` の `headerKebabGroups` に停止ボタン（`id: "abort"`）を追加
- 条件: `!isMd && working`（モバイルでタスク実行中）
- テストを追加して修正を検証

**変更ファイル:**
- `web/src/components/task/TaskView.tsx` - ケバブメニューに停止ボタンを追加
- `web/src/components/task/TaskView.test.tsx` - テストを追加

**コミット:** `57459fa`

### 教訓
- UI のリファクタリング時は、条件付きレンダリングの変更が他のブレークポイントに与える影響を確認する
- モバイルではヘッダーのスペースが限られるため、ケバブメニューへのフォールバックを忘れない
- テストでは、各ブレークポイントでのボタンの存在・不在を確認することが重要

---

## 2026-07-24 iPhone Safariでのモバイル一覧スクロールを安定化

### やったこと
- CodexBarを折り畳んだ後、モバイルサイドメニューのセッション一覧をiPhone Safariでスクロールできなくなる問題を修正した。
- モバイルドロワーに`h-dvh`で明示的な高さを与え、内部flexスクローラーの高さ計算を安定化した。
- ドロワーの高さ・overflow指定を保護する`Sidebar`回帰テストを追加した。

### 判断理由
- `absolute inset-y-0`だけではWebKitで子要素の`h-full`へ確定高さが伝播しない場合があり、CodexBarを折り畳んで一覧が残余領域を使う時に内部スクロールが失われるため。
- `h-dvh`は既存レイアウトでも使用済みであり、safe-areaの余白を維持した最小変更となるため採用した。

### 教訓
- モバイルのflex内スクロールは、`min-h-0`だけでなく、祖先から明示的な確定高さが伝播することをiOS Safariも含めて確認する。

## 2026-07-24 バグハント（第4回）

### やったこと
体系的なコードレビューを実施。前回の第3回で主要な問題を修正済みだったため、今回はより広範囲なチェックを行った。

**チェック項目:**
1. **型チェック（tsc）** - エラーなし ✅
2. **lint（next lint）** - 警告2件のみ（意図的な未使用パラメータ `_req`, `_arg`） ✅
3. **テスト（vitest）** - すべて合格 ✅
4. **コードレビュー** - 重大な問題なし

**確認した項目:**
- `as` キャストの使用（33件）- 型安全性は低いが、外部APIのレスポンスやJSON.parseの結果などやむを得ないケースが多い
- `.json()` のエラーハンドリング（83件）- 多くは `.catch()` や `try-catch` で保護済み
- `setInterval` のクリーンアップ（7件）- すべて適切に `clearInterval` されている
- `addEventListener` のクリーンアップ（49件）- すべて適切に `removeEventListener` されている
- useEffect の依存配列 - 前回の修正で改善済み
- 非同期処理の競合状態 - 重大な問題なし

**結論:**
コードベースは比較的クリーンで、エラーハンドリングも適切。即時修正が必要なバグは発見されず。

### 教訓
- 定期的なバグハントは重要だが、毎回新たなバグが見つかるわけではない
- コードベースの品質が高い状態を維持するには、lint と型チェックの継続的な実行が有効
- `as` キャストの多用は型安全性を低下させるが、外部APIとの連携では避けるのが難しい場合がある

## 2026-07-24 アーカイブ済みタスクをプロジェクト別にグループ化

### やったこと
`TaskView` のアーカイブタブで、タスク一覧をプロジェクトごとにグループ化して表示するよう改修した。

**実装内容:**
- アーカイブ済みタスクを `groupByProject(tasks)` でプロジェクト ID ごとにグループ化
- プロジェクト一覧は `projectOrder` 配列で明示的に指定した順序（`project-a`, `project-b`, `project-c`, …）で表示。`projectOrder` に含まれないプロジェクトは末尾の「未分類」グループに集約
- 各グループ内のタスクは `updatedAt` 降順でソート。`updatedAt` が同値の場合は ID の辞書順で安定ソート
- グループ見出しはプロジェクト名（`project.name`）を表示。プロジェクトが存在しない（削除済み等）場合は ID をフォールバック表示
- React の `key` にはプロジェクト名（`project.name`）ではなくプロジェクト ID（`project.id`）を使用。理由: プロジェクト名はユーザーが変更可能であり、名前変更時に React が全子要素を再マウントしてしまうのを防ぐため。ID は不変なのでキーの安定性が保証される
- 未紐付けタスク（`projectId` が null/undefined/空文字）は「未分類」グループとして常に末尾に表示

**変更ファイル:**
- `web/src/components/TaskView.tsx` — グループ化ロジック・グループ見出し・ソートを追加
- `web/src/components/__tests__/TaskView.test.tsx` — グループ化・ソート順・未分類・キー安定性のテストを追加

### 判断理由
- プロジェクト一覧順を `projectOrder` で固定したのは、ユーザーがよく使うプロジェクトを先頭に表示したいという要件のため。`Object.keys(grouped)` の列挙順は実装依存で信頼できない
- `updatedAt` 降順＋ID 同値順は、最新タスクが常にグループ先頭に来るようにするため。ID で二次ソートすることで `updatedAt` が同一（例: 一括アーカイブ）の場合も表示順が決定論的になる
- 未分類グループを末尾に固定したのは、プロジェクトに属さないタスクが先頭に来ると視認性が低下するため
- React key に `project.id` を使う判断は、プロジェクト名変更時の不要な再レンダリングを防ぐ実践的な理由による

### 教訓
- グループ化 UI では「グループの順序」「グループ内のソート」「未分類の扱い」の3軸を明示的に設計しないと、実装後に要件の抜けに気づく
- React key に表示名を使うのは一見自然だが、名前が可変の場合は不適切。不変 ID をキーにする原則を徹底すべき
- テストではグループ化・ソート順・未分類・キー安定性の4観点をカバーすることで、リファクタリング耐性が高まる

## 2026-07-24 バグハント（第3回）

### やったこと
静的解析（tsc, next lint）とコードレビューを実施し、5件の問題を発見・修正。

**発見した問題（5件）:**
1. **error.tsx 系（2ファイル）** - `"use client";` ディレクティブが重複
   - 修正: 重複を削除
   - コミット: `e28a7da`

2. **HomeView.tsx** - 未使用 import（`Mic`, `MicOff`）
   - 修正: import から削除
   - コミット: `e28a7da`

3. **AttentionQueueModal.tsx** - 不要な `eslint-disable-next-line` ディレクティブ
   - 修正: コメントを削除
   - コミット: `e28a7da`

4. **GlobalAttentionProvider.tsx** - useEffect の依存配列が `items.length` のみで、内容の変化を検出できない（stale closure）
   - 詳細: コメントには「ID の変化を検出する」とあるが、依存配列が `items.length` のみ。配列の内容が変わっても length が変わらなければ実行されない（例: 1つ解決 + 1つ新規追加）
   - 修正: 依存配列を `items` に変更
   - コミット: `e28a7da`

5. **useSessionStream.ts** - 未使用パラメータ `reason`
   - 修正: `_reason` にリネーム（未使用パラメータの慣習）
   - コミット: `e28a7da`

### 教訓
- `"use client";` の重複は構文エラーにはならないが、コードの品質を下げる。lint で検出可能
- useEffect の依存配列は、コメントや実装の意図と一致しているか確認すべき。`items.length` と `items` は意味が異なる
- 未使用変数・パラメータは `_` プレフィックスで明示すると、意図的な未使用が伝わる
...
- defaultTargetがcurrentと一致する場合はnullを返し自己マージを防止（R35#3）— `bfd32ca`
- StatusBadgeにarchivedエントリを追加（R12#2の型エラー修正）— `bfd32ca`
- browse/dirsのfiles=1をallowlist制限しFileTree root超えを防止（R20/R6#2）— `b5adc8a`
- 為替レートを保存前にclampしUIズレを防止（R9#2）— `115fd70`
- AddProjectのmanualPathをナビゲーション中に上書きしない（R9#3）— `115fd70`
- SSE再接続後にREST statusを信頼しstale idle guardを回避（R9#1）— `5e354f7`
- abort後にpreferRestStatusを即座にリセットし再送信時のstale idle guardを防止（R17）— `72aee51`
...
## 2026-07-24 サブエージェント起動の許可設定

### やったこと
- コンポーザーの「フルアクセス」直後に、サブエージェント起動（`task` 権限）の「許可 / 不許可」選択を追加した。
- 選択値を localStorage と CustomEvent で保持・同期し、既定値を「許可」とした。
- 「不許可」時は `task` 権限要求だけを自動拒否し、フルアクセス中でもこの拒否を優先するようにした。
- ユニット・コンポーネントテストを追加し、typecheck と関連テストを通過した。

### 判断理由
- 既存のアクセスモードと同じクライアント側の永続化・自動応答パターンを使うことで、バックエンド設定を変更せず一貫した操作感を保てる。
- 意図しないサブエージェント連鎖を防ぐ安全制御として、不許可はフルアクセスより優先する。

### 教訓
- 個別権限の安全側オーバーライドは、全権限向けの自動承認処理より先に判定し、対象外の権限挙動を明示的に不変に保つ。

---

## 2026-07-24 ChunkLoadError の恒久対策

### やったこと
- 稼働中の本番 WebUI（`next start`）を検出した場合、`build.bat`、`setup.bat`、`npm run build` がビルドを拒否するガードを追加した。
- Service Worker の独自キャッシュから `/_next/` を除外し、キャッシュ名を v3 に更新して古い Next.js チャンクを削除するようにした。
- ビルドガード、バッチ入口、Service Worker のチャンク除外と旧キャッシュ削除を回帰テストで検証した。
- README に安全なビルド手順を追記した。

### 判断理由
- 起動中の `next start` と後から置換された `.next` が別世代になると、実在しない chunk を要求して `ChunkLoadError` が発生するため、上書き自体を防ぐ必要がある。
- `/_next/` を cache-first にすると、デプロイ世代をまたいだ古い chunk が端末に残るため、Next.js の配信・キャッシュ制御に委ねる。

### 教訓
- 本番サーバーが参照中のビルド出力を直接上書きしてはならない。停止・ビルド・起動を直列化するか、稼働中はビルドを失敗させる。
- Service Worker はアプリ固有の静的資産だけをキャッシュし、ハッシュ付きでも世代整合性が必要な Next.js の `/_next/` は独自の cache-first 対象にしない。

---

## 2026-07-24 サブエージェント不許可の実行前適用

### やったこと
- サブエージェント不許可を、保留中の権限要求へのクライアント側 reject だけでなく、OpenCode の実行エージェントに対する `permission.task: deny` として実行前に適用した。
- 任意の設定更新を許さない専用 API を追加し、許可/不許可・対象エージェント・許可済みディレクトリだけを検証して最小の `/config` PATCH を送るようにした。
- 新規タスクは最初の prompt/command より前に設定を反映し、既存タスクの切替にも同じ制御を適用した。

### 判断理由
- `permission.task` が既に allow の場合、OpenCode は確認イベントを出さずに子セッションを作るため、UI 側の自動 reject だけでは実行を防げない。
- 汎用の config 書込みプロキシを解放せず、task 権限の allow/deny に限定した経路にすることで設定改変の攻撃面を増やさない。

### 教訓
- 実行前に評価される権限は、イベントを監視して事後応答するだけでは防御できない。実行エンジンの評価点より前に安全側の規則を適用する。

---

## 2026-07-24 画像対応モデルのフォローアップ送信修正

### やったこと
- BFF プロキシの画像 capability 判定で、ディレクトリ別の provider / agent キャッシュが未投入の場合に OpenCode へライブ照会するフォールバックを追加した。
- directory なしのモデル一覧取得後でも、directory 付きの画像 prompt が画像対応モデルへ正常に転送される回帰テストを追加した。

### 判断理由
- UI の初期モデル一覧取得は directory を渡さないため、従来のキャッシュのみの判定では画像対応 capability を確認できず、フォローアップ送信を常に fail-closed で拒否していた。
- ライブ照会に失敗した場合は従来どおり拒否し、未確認モデルを許可しない安全性を維持した。

### 教訓
- capability のキャッシュを認可判定に使う場合、未シード状態を通常系として扱い、権威データへの安全なフォールバックか明示的なシードを設計する。

---

## 2026-07-24 モバイル会話領域の拡張

### やったこと
- モバイルの共通「OpenCodeWebUI」バーを廃止し、TaskView のセッション名左に44pxのハンバーガーメニューを統合した。
- Home、Settings、TaskView の読み込み・エラー状態、Next.js のエラー画面を含む全モバイル状態にナビゲーション入口を配置した。
- 通知バッジを各モバイルヘッダーへ移し、メニューボタンと通知バッジのキーボードフォーカス表示を既存サイドバーと統一した。
- ドロワーの開閉状態を ShellContext で共有し、既存のフォーカストラップ、Escape、オーバーレイ操作を維持した。

### 判断理由
- タイトル専用バーと会話ヘッダーの二段構成を一段に統合し、会話開始位置を約1バー分上げるため。
- グローバルバーを削除すると読み込み中・エラー時を含むページ境界でナビゲーションが消えるため、例外状態も明示的にカバーした。

### 教訓
- レイアウト共通のモバイル導線をページヘッダーへ移す場合は、通常画面だけでなく loading、error boundary、空状態も受入条件に含める。
- タッチ操作の拡大と同時に、フォーカス可視性・ARIA状態・ドロワーへのフォーカス復帰を回帰テストで確認する。

---

## 2026-07-24 ツール中断の誤エラー表示修正

### やったこと
- OpenCode の `session.next.tool.failed` に含まれる既知の abort/cancel メッセージを、通常エラーではなく終端状態 `cancelled` として扱うようにした。
- UI は赤いエラー表示や生の `Tool execution aborted` を出さず、中立な「中断されました」を表示するようにした。
- REST 再同期と `message.part.updated` でも同じ正規化を行い、SSE で一度直した表示が再同期で error に戻る回帰を防いだ。
- 既知メッセージの完全一致判定、通常エラーの維持、再同期の回帰をテストした。

### 判断理由
- エンジンの失敗イベントには、ユーザー操作による中断か内部キャンセルかを判別できる構造化情報がない。そのため、既知の汎用中断文言だけを小文字化・前後空白除去後の完全一致で `cancelled` に分類し、通常の失敗を隠さないようにした。
- 部分一致ではタイムアウト等の実エラーを誤って中断扱いにするため採用しなかった。

### 教訓
- SSE のイベントだけで UI 状態を補正しても、REST 再同期や別イベントが同じデータを上書きする経路を全て確認しなければ表示は安定しない。状態の正規化はサーバー入力の共通入口で行う。

---

## 2026-07-24 モバイルヘッダー操作のケバブ集約

### やったこと
- `md` 未満のタスクヘッダーから、実行中の停止ボタンとコンテキスト圧縮の直接ボタンを除外した。
- コンテキスト圧縮をケバブメニューへ追加し、既存のタスク・セッション・パネル操作と同じ導線から実行できるようにした。
- モバイルでの非表示、ケバブ内の表示、圧縮操作の実行を回帰テストで確認した。

### 判断理由
- モバイルの限られたヘッダー幅では直接操作を増やさず、既存のケバブに集約するほうがタイトルと主要導線を保ちやすい。
- `md` 以上は既存の直接操作を維持し、デスクトップの操作効率を変えないようにした。

### 教訓
- 操作をケバブへ移す変更では、表示の有無だけでなく、実際のクリックが元のアクションへ接続されていることもテストする。

---

## 2026-07-24 モバイルToDoプランの初期折り畳み

### やったこと
- 作業中にToDoプランが作成されても、`md` 未満では初期状態を折り畳みにした。タップすれば従来どおり詳細を展開でき、`md` 以上では作業中に自動展開する既存挙動を維持した。
- ToDoプランの開閉ボタンに `aria-expanded` を追加し、モバイル初期状態を回帰テストした。

### 判断理由
- モバイルではComposer直上の展開済みプランが入力・会話領域を大きく占有するため、要約バッジだけを初期表示するほうが操作面積を確保できる。
...
## 2026-07-24 NextAction機能（会話履歴から次の指示を提案）

### やったこと
- タスク画面のフォローアップ composer に「次の指示を提案」機能を追加。
- 会話履歴をサーバー側で取得し、ツール無効の一時 OpenCode セッションで1件の日本語指示文を生成。元セッションは変更しない。
- 提案は composer に転記するだけで自動送信しない。利用者が明示的に送信する。
- 新規ファイル: `next-action-text.ts`（pure helper）, `next-action/route.ts`（API）, `NextAction.tsx`（UI）, 各テスト。
- TaskView に統合: idle+会話あり+注意要求なし時だけ表示。会話更新・revert・タスク切替で失効。
- UIレビュー HIGH 2件を修正: `text-fg`→`text-text`（未定義トークン）、エラー文言を固定日本語に統一。
- セキュリティ監査: Critical/High/Medium 0件、Low 2件（ローカル単一ユーザーでは実害限定的）。

### 判断理由
- OpenCode にネイティブの提案 API がないため、一時セッション＋同期 `/prompt`（`POST /session/{id}/message`）で生成。`tools: {}` ではなく `/experimental/tool/ids` で全ツールIDを取得し false に設定（確実な無効化）。
- 会話本文はクライアントから送らずサーバー側で取得（プライバシー・改ざん防止）。
- 提案の自動送信は誤操作リスクが高いため、転記＋明示送信の2ステップを必須とした。

### 教訓
- Tailwind トークンは `globals.css` の `@theme inline` で定義済みのものだけ使う。`text-fg` のような未定義クラスはビルドで無視され、継承で偶然動作するため発見が遅れる。
- API エラーの生テキストを UI にそのまま出すと、日本語 UI に英語が混在する。クライアント側で固定文言に置き換える。
- `docs/superpowers/` は `.gitignore` 対象だが、仕様書・計画書は `git add -f` で強制追跡している。

### コミット
- `6be4b97` 仕様、`6a6fb66` 計画、`8867938` helper、`a2cdfda` API、`4c90725` UI、`bef3501` TaskView統合、`6343347` lint修正、`273e74e` レビュー修正

### 検証
- Vitest 91 files / 826 tests 全通過（新規31件含む）
- ESLint 対象ファイル警告0
- UIレビュー: HIGH 2件修正済み、セキュリティ監査: 合格
