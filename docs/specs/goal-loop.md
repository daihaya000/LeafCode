# Goalループ 状態機械の定義と監査是正

## 背景

Goalループ（`web/src/lib/goal-loop.ts`、約1280行）は本リポジトリで最も複雑な状態機械だが、
...
1. Goalループの状態・遷移・不変条件・ターン予算を、実装に先立つ正規の定義として確定する。
...
- 非対象: GoalループのUIデザイン刷新（`docs/goal-loop-ui-redesign.md` の範囲）。
...
- **I8**: 履歴（`/session/:id/message`）の読取失敗を「空・idle」とみなしてはならない。
  読めない間は送信も結果適用もしない。
- **I9**: Goalループ自身のプロンプトは `ocServer` で OpenCode エンジンへ直送し、
...
| `unreadable_result` | 結果JSONが読めなかった | 「Goalループの結果JSONを読めなかったため一時停止しました。」 |
...
2. `web/src/app/api/opencode/[...path]/route.ts` の `proxy` に、
   ...
   - 対象ループが非終端で、かつ pause が成立しなかった場合は転送せず `409` を返す。
     本文は `{ error: "Goalループを一時停止できないため手動送信を中止しました。…" }`。
...
- `PROMPT_TIMEOUT_MS` のコメントは「BFF の上限 290s まで許容する」と述べているのに値は `120_000`。
  ...
  長時間ミューテーション用の上限）であり、`ocServer` で直送する Goalループのプロンプト送信には
  適用されない。
