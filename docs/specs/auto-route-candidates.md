...
┌ ライト（短い質問・雑談）                          [リセット] ┐
│ 候補（上が優先）                                            │
│ 1. [モデル指定▾][gemini-2.5-flash ▾][minimal ▾]  ↑ ↓ ✕      │
│ 2. [コスト帯  ▾][低コスト         ▾][自動    ▾]  ↑ ↓ ✕      │
│ [＋ 候補を追加]                                             │
│ ▸ effort フォールバック順（プリセット）                      │
│ 全候補が使えない時: [プリセットに従う ▾]                     │
...
- **候補行**: 種別ドロップダウン（`モデル指定` / `コスト帯` / `最強候補`）で
  行の形が切り替わる。
  - `モデル指定`: モデルドロップダウン + effort ドロップダウン
  - `コスト帯`: コスト帯ドロップダウン（低 / 中 / 高）+ effort
  - `最強候補`: effort のみ
- **モデルドロップダウン**: `ProviderModelsSettings` の既存
  ...
  設定済みだが現在無効 / 未接続のモデルは `(未接続)` 付きで選択肢に残す
  （設定を消さずに保持できるようにする）。
- **effort ドロップダウン**: 選択モデルの `getIntelligenceVariants` の結果のみ。
  先頭に `自動`（`variant` キー無し）、次に `指定なし`（`""`）。
  `コスト帯` / `最強候補` の行はモデルが未確定なので
  `ALL_INTELLIGENCE_VARIANTS` を出す。
- **並べ替え**: ↑ ↓ ボタン（現行踏襲。ドラッグ&ドロップは非対応）。
- **削除**: ✕。全削除すると候補 0 件 = プリセット使用となり、
  `プリセットを使用中` を明示表示する。
- **追加**: 末尾に追加。既定値は `presetTierRoute(mode, tier).candidates[0]`、
  それが既にリストにあれば `{ kind: "cost", cost: "mid" }`。
  `MAX_AUTO_ROUTE_CANDIDATES` に達したらボタンを disabled にする。
- **effort フォールバック順**: 折りたたみ。中身は現行 `VariantOrderEditor`
  相当（チェック + 上下）。未編集はプリセット表示。
...
- モードタブ切替で表示候補が変わる
- 候補の追加 / 削除 / 並べ替え / 種別切替が `onChange` に反映される
