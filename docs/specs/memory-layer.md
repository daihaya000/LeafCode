...
メモリ層全体のマスタースイッチを `memory.enabled` に持つ。`"0"` で無効、`"1"` で有効、
**未設定は有効**(この機能より前のインストールの挙動を変えないため)。設定の読み取りに
失敗した場合も有効とみなす。無効化はユーザーの明示的な選択であるべきで、読み取り失敗の
副作用で起きてはならない。

無効時の挙動:

- 注入しない(`memoryInjectionFor` は空文字、`claimMemoryInjectionForSession` は `null`)。
  無効中は claim 行を書かないため、再有効化後に同じセッションへ通常どおり注入できる。
- 書き込まない(`insertExtractedMemories` は全カウント0で返す)。自動抽出も走らない。
- `POST /api/memory/extract` は `409` で拒否する。モデル呼び出し前に止めることで、
  書き込めない行のために課金しない。
- **保存済みのメモリは削除しない**。一覧・編集・削除・整理はそのまま使える。
  無効化は「止める」であって「消す」ではない。

一括削除 `deleteAllMemories({ workspaceId })` はスコープ内の全行を削除する。

- UI の確認ダイアログでプロジェクト名と件数(使用中/候補の内訳)を提示してから実行する。
  取り消しはできない。
- API は `POST /api/memory/purge` に `confirm: true` を必須とする(誤送信で消えない)。
- 監査ログは1操作につき1行(`detail='purge scope=<key> count=<n>'`)。
  1ユーザー操作で数千行の監査を作らない。削除された内容はどちらにせよ復元できない。
- メモリ層が無効でも実行できる。「無効にして掃除する」は自然な流れであり、
  掃除のために再有効化させない。
...
| `POST /api/memory/purge` | スコープ内のメモリを全削除。`confirm: true` 必須 |
| `GET/PUT /api/settings/memory.enabled` | メモリ層のマスタースイッチ(`"0"`/`"1"`、未設定=有効) |
...
- 「メモリ機能を使う」トグル(`memory.enabled`)。OFF時は注入・抽出が止まる旨と、
  保存済みメモリが残る旨を明示する。OFF中は保存前確認トグルと抽出ボタンを無効化する
  (APIが拒否するため、無駄な往復を誘わない)。
...
- 「すべて削除(N件)」ボタン(danger)。確認ダイアログで承諾後に `POST /api/memory/purge`。
...
- マスタースイッチ: 無効時に注入・claim・抽出書き込みが止まり保存済み行は残る、
  未設定は有効扱い、再有効化後に同じセッションへ注入できる、`extract` APIが409を返し
  モデルを呼ばない、設定APIが `"0"`/`"1"` 以外を拒否する。
- 一括削除: スコープ内全削除と他スコープ非影響、監査ログ1行、空スコープはno-op、
  無効化中でも削除できる、`confirm` 無しは400、UIのキャンセルで削除しない。
