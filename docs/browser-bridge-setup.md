...
5. 内容を確認し「許可」をクリック
6. popup のステータスが「ローカル Broker に接続済み」になれば成功（拒否した場合は「未ペアリング」に戻る）

### ペアリングの仕組み

- ペアリング要求はソケット接続ごとに 1 件だけ保留される。承認・拒否・拡張の切断・一定時間の未決定でいずれも失効する（コードの入力・共有は一切発生しない）
- 許可すると Broker が device key を発行し、拡張機能は `chrome.storage.local` に保存して次回以降は自動再接続する
...
- Broker が device key を認識できなくなった場合（例: 永続化されていない状態での host 再起動）、拡張は古い key を破棄して自動的に新しいペアリング要求を送り直す。WebUI で再度「許可」するだけで復旧する
- ペアリング解除は拡張 popup の「この接続を削除」から
...
- 既定では新規タブは自動共有されない。都度「現在のタブを共有」が必要（下記「自動共有」を有効にした場合を除く）
...
- 拡張 popup の各タブ行にある「〜の共有を停止」ボタンで個別解除
- 拡張 popup の「この接続を削除」で全タブ共有解除 + ペアリング解除
- タブを閉じる・ナビゲーションでも自動解除

### 自動共有（オプション・既定オフ）

毎回「現在のタブを共有」をクリックする代わりに、閲覧中のタブを自動的に共有できます。

1. 拡張 popup の「閲覧中のタブを自動的に共有」トグルを ON にする
...
- [ ] unpacked 拡張を読み込み、コード入力なしで自動ペアリング要求を送信し、WebUI で許可すると接続できる
- [ ] WebUI でペアリング要求を「拒否」すると device key が発行されず、拡張は未ペアリングのままになる
...
- ペアリングはコード入力ではなく WebUI でのワンクリック明示許可が必須。要求は 1 ソケットにつき 1 件だけ保留され、承認・拒否・切断・タイムアウトのいずれかで失効する
