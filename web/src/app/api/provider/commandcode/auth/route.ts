...
export async function POST(req: Request) {
  ...
  if (typeof body?.key !== "string" || body.key.trim().length === 0 || body.key.length > 4096) {
    return NextResponse.json({ error: "CommandCode Proxy認証キーを入力してください" }, { status: 400 });
  }
  try {
    ...
    return NextResponse.json({ error: "CommandCode Proxy認証キーの保存に失敗しました" }, { status: 500 });
  }
}
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ error: "CommandCode Proxy認証の解除に失敗しました" }, { status: 500 });
  }
}
