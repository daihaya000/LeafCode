...
async function supportsImageInput(
  ...
  try {
    ...
    // agent's pinned model only applies when the request carries none.
    if (!effectiveModel && agentName) {
  ...
  if (files.length > 0 && !(await supportsImageInput(effectiveModel, body?.agent))) {
    ...
    if (!qwenNativeFallback) {
      return NextResponse.json(
        {
          error:
            "選択中のモデルは画像入力に対応しておらず、画像事前解析も有効ではありません。画像対応モデルを選ぶか、設定の「モデル」タブで画像事前解析モデルを選択してください。",
        },
        { status: 400 },
      );
    }
