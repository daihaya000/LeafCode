...
const { getWorkspace, latestBindings, listSessionBindings, setSessionSkillPermission } =
  vi.hoisted(() => ({
    getWorkspace: vi.fn(),
    latestBindings: vi.fn(),
    listSessionBindings: vi.fn(),
    setSessionSkillPermission: vi.fn(),
  }));
...
vi.mock("@/lib/opencode-skill-permission", () => ({ setSessionSkillPermission }));

import { POST } from "./route";

function request(body: unknown) {
  return new NextRequest("http://localhost/api/skill-permission", {
    method: "POST",
...
describe("POST /api/skill-permission", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("applies the skill ruleset to the task's live OpenCode session", async () => {
  ...
  it("returns 404 when sessionId is not bound to the task", async () => {
    ...
    expect(setSessionSkillPermission).not.toHaveBeenCalled();
  });
