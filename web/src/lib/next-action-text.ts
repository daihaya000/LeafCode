...
export const NEXT_ACTION_SYSTEM_INSTRUCTION = [
  ...
  "- 会話の最後の状態を読み、目的や成果物に直接つながる具体的な動作を選ぶ",
  "- 未回答の質問・未解決の失敗・残タスクがあれば最優先で触れる",
  "- 基本は作業を前に進める次工程（実装・修正・調査・整理・文書化など）を提案する",
  "- 確認・テスト・レビュー・コミットは、会話中で必要性が明確な場合に限る。単なる確認作業を汎用的な次の一手として提案しない",
  "- 作業が完了している場合も、対象のない確認で終わらせず、次に価値を生む具体的な工程を提案する",
  "- 「確認してください」「対応してください」のように対象や動作が曖昧な指示は禁止",
  "- 直近の成果を踏まえ、まだ価値のある未完了作業があればそれを優先する",
