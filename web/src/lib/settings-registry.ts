/**
 * 設定スキーマのレジストリ（REFACTORING_PLAN P4-d / IMPROVEMENT 2-2）。
 * 許容キー・boolean 化・キー別バリデーションを 1 箇所で宣言し、
 * `/api/settings/[key]` ルートはそれを参照するだけにする。
...
export const MAX_SETTING_VALUE_CHARS = 32_768;
...
export function isAllowedKey(key: string): key is string {
