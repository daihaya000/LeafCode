 ...
 *
 * The v1 layer deduplicated by exact string match only, which never fires in
 * practice: an LLM re-extracting the same transcript emits a paraphrase, not
 * the same sentence (production: 2,356 rows, zero exact duplicates, 407
 * paraphrase duplicates in the worst scope).
 *
 * Three mechanisms replace it, in increasing cost order:
 *
 * 1. `normalizeMemoryKey` — canonical key stored on every row
 *    (`memories.norm_key`). Catches formatting/politeness-only differences with
 *    an index lookup.
 * 2. `memoryIdentifiers` — the ASCII identifier set (file names, commands,
 *    flags). Two propositions about *different* identifiers are never merged,
 *    which is what keeps "MEMORY.md is gitignored" and "LESSONS.md is
 *    gitignored" apart even though their prose is nearly identical.
 * 3. `trigramSimilarity` — character-trigram Jaccard over the normalized text.
 *    Unlike FTS5 bm25 (an unbounded relevance score) Jaccard is a normalized
 *    0..1 similarity, so fixed thresholds are well-defined.
 *
 * Merging is deliberately conservative: losing a distinct memory is worse than
 * keeping a near-duplicate, and the primary duplicate source is fixed upstream
 * by incremental extraction. This layer is the backstop, not the main defense.
 *
...
/**
 * Prose-similarity bar when both sides talk about the same subject, i.e. their
 * identifier sets overlap by at least {@link MEMORY_IDENTIFIER_OVERLAP}.
 * Paraphrases of one proposition ("`MEMORY.md` は追跡対象から外す" /
 * "`MEMORY.md` は追跡対象から除外されている") land here.
 */
export const MEMORY_SIMILARITY_SAME_IDENTIFIERS = 0.6;

/**
 * Identifier-set Jaccard above which two contents are considered to be about
 * the same subject. Tuned so `{MEMORY.md, .gitignore, git}` vs
 * `{MEMORY.md, .gitignore}` (0.67) counts as the same subject while
 * `{MEMORY.md, .gitignore, git}` vs `{LESSONS.md, .gitignore, git}` (0.50)
 * does not.
 */
export const MEMORY_IDENTIFIER_OVERLAP = 0.6;

/**
 * Bar when the identifier sets differ (or one side has none). Only
 * near-verbatim rewrites are merged, because differing identifiers usually mean
 * a genuinely different proposition.
 */
export const MEMORY_SIMILARITY_DIFFERENT_IDENTIFIERS = 0.85;

/** Bar when neither side contains identifiers (pure prose propositions). */
