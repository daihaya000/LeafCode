...
export function ClaudeSubscriptionAuth({ showHeading = true }: { showHeading?: boolean }) {
  ...
  const load = () => {
    setState("loading");
    void getJson<AuthResponse>("/api/provider/claude/auth")
      .then((result) => {
  ...
  return (
    <section aria-label={showHeading ? undefined : "Claude CLI Proxy"} aria-labelledby={showHeading ? "claude-cli-proxy-heading" : undefined}>
      ...
      <div className="rounded-xl border border-border bg-surface px-4 py-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-xs text-faint">
              Claude CLIをローカルプロキシ経由で使用します。認証情報はClaude CLI側で管理されるため、LeafCodeでAPIキーを入力・保存する必要はありません。
            </p>
          </div>
        ...
        <p className="mt-3 text-xs text-muted">
          未認証の場合はターミナルで <code className="rounded bg-bg px-1.5 py-0.5 font-mono text-text">claude login</code> を実行し、認証後に再確認してください。
        </p>
      </div>
    </section>
  );
