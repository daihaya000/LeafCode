...
export function ClaudeSubscriptionAuth({
  showHeading = true,
}: {
  showHeading?: boolean;
}) {
  const auth = useSubscriptionOAuth({
    ...
    authorizeEndpoint: "/api/provider/anthropic/oauth/authorize",
    popupName: "claude-auth",
    findMethodIndex: (methods: AuthMethod[]) =>
      methods.findIndex((method) => method.type === "oauth"),
    isConnected,
    notAvailableMessage: "ClaudeのOAuth認証方式が利用できません",
    loadErrorMessage: "Claudeの認証状態を取得できませんでした",
    timeoutMessage: "認証完了を確認できませんでした。認証後に再確認してください。",
    startErrorMessage: "Claudeの認証を開始できませんでした",
  });
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-xs text-faint">
              Claude CLIをローカルプロキシ経由で使用します。APIキーを入力せず、Anthropicのアカウントをブラウザで認証します。
            </p>
          </div>
