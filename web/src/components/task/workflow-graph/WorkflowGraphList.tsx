    ...
    <aside className="min-w-0 rounded-lg border border-border bg-surface p-3" aria-label="Workflow Nodeと接続の一覧">
      ...
      <ul className="mt-2 space-y-1">
        ...
        {graph.nodes.map((node) => {
          ...
          const status = state?.status ?? (node.type.startsWith("control.") ? "ready" : "ready");
          return (
            <li key={node.id}>
              <button
                type="button"
                className={cx(
                  "flex min-h-10 w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-xs transition-colors hover:bg-surface-2 focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-primary",
